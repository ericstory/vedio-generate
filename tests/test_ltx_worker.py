...
def test_gpu_policy_stays_inside_cost_and_vram_limits() -> None:
    ...
    assert policy["maximum_secure_price_usd_per_hour"] == 2.1
    assert all(gpu["vram_gb"] >= 48 for gpu in policy["gpu_types"])
    assert all(gpu["secure_price_usd_per_hour"] <= 2.1 for gpu in policy["gpu_types"])
    assert not any("H100" in gpu["id"] or "H200" in gpu["id"] for gpu in policy["gpu_types"])
