...
WORKER_LOG = Path(os.environ.get("LTX_WORKER_LOG", "/tmp/ltx-worker.log"))
...
def _tee_process_output() -> None:
    ...
    RunPod discards the container log together with a one-shot Pod, so a
    failure callback that only carries `str(exc)` loses whatever the CUDA
    runtime, natten or ffmpeg wrote to stderr just before. Routing fds 1 and 2
    through `tee` keeps the container log intact while giving the failure
    callback a tail to quote.
    """
    try:
...
def _log_tail(limit: int = 3000) -> str:
    try:
        sys.stdout.flush()
        sys.stderr.flush()
        # Give tee a moment to drain what was written last.
        time.sleep(1)
...
def _await_deletion() -> None:
    # The control plane deletes the billed Pod after committing the callback.
    # Keeping the container alive prevents RunPod from restarting a completed
    # one-shot job.
    while True:
        time.sleep(60)
...
def main() -> None:
    ...
    _tee_process_output()
    if not run_job("ltx-pod-smoke", params):
        _await_deletion()
    url = jobs_url()
